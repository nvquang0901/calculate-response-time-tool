...
import tempfile, os, zipfile, csv
import asyncio
from services.evaluate import evaluate_image, evaluate_folder,evaluate_folder_zip
import aiohttp
from services.chat_service import send_chat, send_tts
from services.utils import extract_zip_to_temp
import tempfile, zipfile, aiohttp, os, asyncio, csv, io, pandas as pd
...
# Streamlit config
st.set_page_config(page_title="LLM Eval Tool", page_icon="🧠", layout="centered")
st.title("🧠 Evaluate Chat/TTS & Image+Chat Response Time")
...
env_choice = st.selectbox("🌍 Chọn môi trường:", list(domain_options.keys()))
...
if mode == "Texts":
    ...
    input_method = st.radio("Chọn phương thức nhập:", ["Nhập trực tiếp", "Tải file CSV/TXT"])
    prompts = []
    ...
    if input_method == "Nhập trực tiếp":
        ...
        uploaded_file = st.file_uploader("Tải file CSV hoặc TXT", type=["csv","txt"])
        if uploaded_file:
    ...
    if start_eval:
        if not prompts:
            ...
            with pd.ExcelWriter(buffer, engine="openpyxl") as writer:
                df_results.to_excel(writer, index=False, sheet_name="Chat Results")
            buffer.seek(0)

            st.download_button(
                "⬇️ Tải file Excel",
                data=buffer,
                file_name="text_eval.xlsx",
    ...
    # Chọn kiểu xử lý
    eval_type = st.radio("Chọn kiểu đánh giá:", ["Random ghép ảnh với file .txt", "Theo Folder ZIP"])
    ...
    if eval_type == "Random ghép ảnh với file .txt":
        ...
        if uploaded_imgs and uploaded_txt:
            ...
            if total_prompts == 0:
                ...
                if st.button("🚀 Bắt đầu đánh giá Random"):
                    ...
                    st.download_button(
                        "⬇️ Tải file Excel",
                        data=buffer,
                        file_name="image_text_random.xlsx",
                        mime="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet"
                    )
        ...
        if uploaded_zip and st.button("🚀 Bắt đầu đánh giá Folder"):
            ...
            if not results:
                ...
                st.download_button(
                    "⬇️ Tải file Excel",
                    data=buffer,
                    file_name="chat_response_results.xlsx",
                    mime="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet"
                )
